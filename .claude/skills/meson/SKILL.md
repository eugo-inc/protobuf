...
## Eugo native dependencies

Packaging a C/C++ lib under `dependencies/native/<name>/` in this repo? This skill covers upstream Meson mechanics only. Eugo's conventions — the `EUGO_MESON_COMMON_OPTIONS` env, `MESON_LOG_LEVEL=debug`, the 5-category flag taxonomy, `meson compile -C eugo_build -v` — live in `../native-dependency/MESON.md`. Start at `../native-dependency/SKILL.md`.

## Related skills

- Use `../cmake/SKILL.md` when CMake is required or preferred
- Use `../ninja/SKILL.md` — Meson uses Ninja as its backend
- Use `../gcc/SKILL.md` or `../clang/SKILL.md` for compiler flag context
